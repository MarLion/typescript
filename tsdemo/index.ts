...
// 4.数组类型(array)  有两种定义数组的方式
 ...
 // 5.元祖类型(tuple) 属于数组的一种 可以指定数组中每个元素的类型

 let arr3:[string,number,boolean] = ['1',1,true]

 // 6.枚举类型(enum) 主要是用来定义标识符
 ...
 let z:Exc = Exc.orage
 console.log(z) //Color.red输出为0 Color.bule输出为1 Color.orage输出为2 在bule的基础上加1
...
// 10.never 类型：是其他类型（包含null 和 undefined）的子类型 代表重不会出现的值 声明never的变量只能被never类型赋值
